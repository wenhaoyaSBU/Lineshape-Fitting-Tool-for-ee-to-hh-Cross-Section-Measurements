#pragma once
// sigma2bodyW6_isr_qf5.hpp
//
// ISR-corrected two-body cross section with the 0th-order Born prefactor
//   sigma_Born,noqf(W) ~ W^{-6}
...
//   Through 5th order the required objects are
//
//     continuum:   I3, I5, I4, I8, I0, I7,
//     resonance:   J1, K1, I1, I6, L1, M1,
//     interference second kernels: J2, K2, J1, K1, I1, I6,
//
//   where
//     J1 = ∫ F /[(1-x)   (A+Bx)],
//     J2 = ∫ F /[(1-x)^2 (A+Bx)],
...
//     M1 = ∫ F (1-x)^(3/2)/(A+Bx)   = ((A+B) I6 - I9)/B.
//
// Therefore everything still closes within I0..I11; no new basis integral is
// required.
...
#include "sigma2bodyW6_isr.hpp"
#include "sigma2bodyW7_isr.hpp"  // for HalfShiftedKernels + builder

namespace isr_sigma2bodyW6 {

using HalfShiftedKernels = isr_sigma2bodyW7::HalfShiftedKernels;
using isr_sigma2bodyW7::build_half_shifted_kernels;
...
inline cd build_threehalf_positive_kernel(const isr_i0i11::Result& I,
                                          cd Aprop,
                                          cd Bprop) {
  // M1 = ∫ F (1-x)^(3/2)/(A+Bx) = ((A+B) I6 - I9)/B
  return (((Aprop + Bprop) * I.I6) - cd(I.I9, 0.0)) / Bprop;
}

inline Corr12345Parts sigma2bodyW6_ISR_corr12345(double W,
                                                 double M,
                                                 double Acal,
                                                 double C1,
                                                 double C2,
                                                 ...
                                                 const IsCache& cache = IsCache{})
{
  ...
  if (!(W > 0.0) || !(M > 0.0)) {
    throw std::invalid_argument("sigma2bodyW6_ISR_corr12345: require W>0 and M>0.");
  }
  if (!(c.Wmin > 0.0 && c.Wmin < W)) {
    throw std::invalid_argument("sigma2bodyW6_ISR_corr12345: require 0 < Wmin < W.");
  }
  ...
  const cd   M1 = build_threehalf_positive_kernel(I, Aprop, Bprop);
  ...
  // ---------------- Order 1 ----------------
  {
    out.ord1.sigmaC = q1 * (4.0 * PI * alpha * alpha * A2 / (3.0 * W5)) * I.I5;

    const double prefR1 = q1 * h0 * (6.0 * PI * A2 * gprod) / (c.Gamma * M5 * W);
    out.ord1.sigmaR = prefR1 * (2.0 * std::real(K.K1));
  ...
  // ---------------- Order 2 ----------------
  {
    out.ord2.sigmaC = q2 * (4.0 * PI * alpha * alpha * A2 / (3.0 * W4)) * I.I4;

    const double prefR2 = q2 * h0 * (6.0 * PI * A2 * gprod) / (c.Gamma * M5);
    out.ord2.sigmaR = prefR2 * (2.0 * std::real(I.I1));
  ...
  // ---------------- Order 3 ----------------
  {
    out.ord3.sigmaC = q3 * (4.0 * PI * alpha * alpha * A2 / (3.0 * W3)) * I.I8;

    const double prefR3 = q3 * h0 * (6.0 * PI * A2 * gprod) * W / (c.Gamma * M5);
    out.ord3.sigmaR = prefR3 * (2.0 * std::real(I.I6));
  ...
  // ---------------- Order 4 ----------------
  {
    out.ord4.sigmaC = q4 * (4.0 * PI * alpha * alpha * A2 / (3.0 * W2)) * I.I0;

    const double prefR4 = q4 * h0 * (6.0 * PI * A2 * gprod) * W2 / (c.Gamma * M5);
    out.ord4.sigmaR = prefR4 * (2.0 * std::real(L1));

    const double prefI4 = q4 * (4.0 * PI * alpha * A2 * gsqrt) * W4 / (c.Gamma * M4);
  ...
  // ---------------- Order 5 ----------------
  {
    out.ord5.sigmaC = q5 * (4.0 * PI * alpha * alpha * A2 / (3.0 * W)) * I.I7;

    const double prefR5 = q5 * h0 * (6.0 * PI * A2 * gprod) * W3 / (c.Gamma * M5);
    out.ord5.sigmaR = prefR5 * (2.0 * std::real(M1));

    const double prefI5 = q5 * (4.0 * PI * alpha * A2 * gsqrt) * W5 / (c.Gamma * M4);
    const cd inner = cd(h1 / W2, 0.0) * M1
                   + cd(mix / W4, 0.0) * I.I6;
...
inline SigmaParts sigma2bodyW6_ISR_qf5(double W,
                                       double M,
                                       double Acal,
                                       double C1,
                                       double C2,
...
{
  if (!(W > 0.0) || !(M > 0.0)) {
    throw std::invalid_argument("sigma2bodyW6_ISR_qf5: require W>0 and M>0.");
  }
  if (!(c.Wmin > 0.0 && c.Wmin < W)) {
    throw std::invalid_argument("sigma2bodyW6_ISR_qf5: require 0 < Wmin < W.");
  }
  ...
  auto base = sigma2bodyW6_ISR(W, M, Acal, C1, C2, phi, Phi, c, iopt, vp, cache);
  auto corr = sigma2bodyW6_ISR_corr12345(W, M, Acal, C1, C2, phi, Phi,
                                         q1, q2, q3, q4, q5,
                                         c, iopt, vp, cache);
