...
//
//   q_f^3(W) \approx q0 + q1 W + q2 W^2 + q3 W^3 + q4 W^4 + q5 W^5.
//
// Before adding the 4th and 5th corrections, we verify that no basis integral
// beyond I0..I11 is needed. This is indeed the case:
//   - continuum kernels require I10, I11, I3, I5, I4, I8,
//   - resonance kernels require J2, K2, J1, K1, I1, I6,
//   - interference kernels require (J3,J2), (K3,K2), (J1,J2), (K1,K2),
//     (I1,J1), (I6,K1),
// all of which are generated from I0..I11.
//
// The recursive shifted kernels are
//   J1 = ∫ F /[(1-x)   (A+Bx)],
//   J2 = ∫ F /[(1-x)^2 (A+Bx)],
//   J3 = ∫ F /[(1-x)^3 (A+Bx)],
//   K1 = ∫ F /[(1-x)^(1/2) (A+Bx)],
...
inline Corr12345Parts sigma2body_ISR_corr12345(double W,
                                               double M,
                                               double Acal,
                                               double C1,
                                               double C2,
                                               ...
                                               const IsCache& cache = IsCache{})
{
  ...
  if (!(W > 0.0) || !(M > 0.0)) {
    throw std::invalid_argument("sigma2body_ISR_corr12345: require W>0 and M>0.");
  }
  if (!(c.Wmin > 0.0 && c.Wmin < W)) {
    throw std::invalid_argument("sigma2body_ISR_corr12345: require 0 < Wmin < W.");
  }
  ...
  // ---------------- Order 4 ----------------
  {
    out.ord4.sigmaC = q4 * (4.0 * PI * alpha * alpha * A2 / (3.0 * W4)) * I.I4;

    const double prefR4 = q4 * h0 * (6.0 * PI * A2 * gprod) / (c.Gamma * M5);
    out.ord4.sigmaR = prefR4 * (2.0 * std::real(I.I1));

    const double prefI4 = q4 * (4.0 * PI * alpha * A2 * gsqrt) * W2 / (c.Gamma * M4);
  ...
  // ---------------- Order 5 ----------------
  {
    out.ord5.sigmaC = q5 * (4.0 * PI * alpha * alpha * A2 / (3.0 * W3)) * I.I8;

    const double prefR5 = q5 * h0 * (6.0 * PI * A2 * gprod) * W / (c.Gamma * M5);
    out.ord5.sigmaR = prefR5 * (2.0 * std::real(I.I6));

    const double prefI5 = q5 * (4.0 * PI * alpha * A2 * gsqrt) * W2 * W / (c.Gamma * M4);
...
inline std::vector<double> convert_p2q(double p0, double p1, double p2, double p3, double p4, double p5, double W0) {
...
inline SigmaParts sigma2body_ISR_qf5(double W,
                                     double M,
                                     double Acal,
                                     double C1,
                                     double C2,
...
{
  if (!(W > 0.0) || !(M > 0.0)) {
    throw std::invalid_argument("sigma2body_ISR_qf5: require W>0 and M>0.");
  }
  if (!(c.Wmin > 0.0 && c.Wmin < W)) {
    throw std::invalid_argument("sigma2body_ISR_qf5: require 0 < Wmin < W.");
  }
  ...
  const auto corr = sigma2body_ISR_corr12345(W, M, Acal, C1, C2, phi, Phi,
                                             q1, q2, q3, q4, q5,
                                             c, iopt, vp, cache);
