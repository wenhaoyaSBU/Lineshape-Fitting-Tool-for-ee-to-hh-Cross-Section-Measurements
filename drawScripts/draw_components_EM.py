...
parser.add_argument("--linear", action='store_true', help="Whether the linear approx is used in chi2 construction.", default=False)
...
linear = parser.parse_args().linear
dof = parser.parse_args().dof
if addbr:
    dof += 1

if addbr:
    print("Including Br pull in the plot.")
if linear:
    print("Using linear approximation for chi2 construction.")
...
fittedFile = "../output/fittedParameters_EM.txt"
lines = []
params_postFit     = []
params_postFit_err = []
...
fittedMfile = "../output/fittedM_EM.txt"
MdevsAbs, Mfitted = [], []
MerrPre = []
MerrPost = []
...
# -----------------------------
theoPoints              = np.loadtxt('../output/getpoint_EM.txt')
theoPoints_con          = np.loadtxt('../output/getpoint_dressed_con_EM.txt')  # continuum component
theoPoints_res          = np.loadtxt('../output/getpoint_dressed_res_EM.txt')  # resonance component
theoPoints_int          = np.loadtxt('../output/getpoint_dressed_int_EM.txt')  # interference component
# theoPoints_res_gammagg  = np.loadtxt('../output/getpoint_dressed_res_gammagg_EM.txt')  # resonance gammagg component
...
for line in other_lines:
    if line.strip():
        ...
        if not linear:
            ...
            MerrRel = MerrPre[lineidx] / Mfitted[lineidx]
            # errTotal = np.sqrt(err_stat**2 + err_syst_unc**2 + err_syst_cor**2 + MerrRel**2)
            errTotal = np.sqrt(err_stat**2 + err_syst_unc**2 + err_syst_cor**2)
...
ax_inset = fig.add_axes([0.17, 0.58, 0.4, 0.21])
...
ax_dev.plot([xlow, xhigh], [0, 0], color='grey', linestyle='--', linewidth=1)  
...
# Save figure
fig.savefig(outDir + 'fitsta_with_pull_LC_components_EM_' + postfix + '.pdf')
fig.savefig(outDir + 'fitsta_with_pull_components_EM_' + postfix + '.pdf')

print("Fitting result saved to " + outDir + "fitsta_with_pull_components_EM_" + postfix + ".pdf")
